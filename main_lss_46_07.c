...
int main(int argc, char* argv[])
{
	...
	int k = 1;
	double *A, *B, *X, *tmp;
	char *filenameIn = "lss_46_07_in.txt";
	char *filenameOut = "lss_46_07_out.txt";
	FILE *in, *out;
	beginTime = clock();
	debug = error = 0;
	...
	if (fscanf(in, "%d", &n) != 1 || n < 0)
	{
		...
		if (err)
		{
			fprintf(stderr, "Oshibka pri razmernosti matrici libo razmer matrichi menishe nulya\n");
		}
		return -1;
	}
	...
	tmp = (double*)malloc((n*n + n*n) * sizeof(double));
